...
#[test]
fn one_variant_tagged_union() {
    let d = witx::parse(
        "(typename $tag (enum u8 $c))
         (typename $u (tagged_union $tag (field $c u8)))",
    );
    assert!(d.is_ok());
}

#[test]
fn two_variant_tagged_union() {
    let d1 = witx::parse(
        "(typename $tag (enum u8 $a $b))
         (typename $u (tagged_union $tag (field $a u8) (field $b u16)))",
    );
    ...
    let d2 = witx::parse(
        "(typename $tag (enum u8 $a $b))
         (typename $u (tagged_union $tag (field $b u16) (field $a u8)))",
    );
...
#[test]
fn empty_variant_tagged_unions() {
    let d1 = witx::parse(
        "(typename $tag (enum u8 $a $b))
         (typename $u (tagged_union $tag (empty $a) (field $b u16)))",
    );
    assert!(d1.is_ok(), "d1 is ok");

    let d2 = witx::parse(
        "(typename $tag (enum u8 $a $b))
         (typename $u (tagged_union $tag (empty $a) (empty $b)))",
    );
    assert!(d2.is_ok(), "d2 is ok");
}
...
#[test]
fn no_tag_tagged_union() {
    let d = witx::parse("(typename $u (tagged_union $tag (field $a u8) (field $b u16)))");
    assert!(d.is_err());
}

#[test]
fn wrong_kind_tag_tagged_union() {
    let d = witx::parse(
        "(typename $tag u32)
            (typename $u (tagged_union $tag (field $a u8) (field $b u16)))",
    );
...
#[test]
fn bad_field_tagged_unions() {
    let d = witx::parse(
        "(typename $tag (enum u8 $c))
         (typename $u (tagged_union $tag (field $b u8)))",
    );
    let (name, reason) = tagged_union_field_err(d).expect("bad field union 1");
    ...
    let d = witx::parse(
        "(typename $tag (enum u8 $c))
         (typename $u (tagged_union $tag (field $c f32) (field $b u8)))",
    );
    let (name, reason) = tagged_union_field_err(d).expect("bad field union 2");
    ...
    let d = witx::parse(
        "(typename $tag (enum u8 $c $d))
         (typename $u (tagged_union $tag (field $c f32)))",
    );
    let (name, reason) = tagged_union_field_err(d).expect("bad field union 3");
...
fn tagged_union_field_err(r: Result<witx::Document, witx::WitxError>) -> Option<(String, String)> {
    match r {
        Err(witx::WitxError::Validation(witx::ValidationError::InvalidTaggedUnionField {
            name,
            reason,
            ..
        })) => Some((name, reason)),
        Err(e) => {
            eprintln!(
                "expected InvalidTaggedUnionField ValidationError, got: {:?}",
                e
            );
            None
        }
        Ok(_) => {
            eprintln!("expected InvalidTaggedUnionField ValidationError, got: Ok(witx::Document)");
            None
        }
    }
}
